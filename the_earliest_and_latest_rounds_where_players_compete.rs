...
pub fn earliest_and_latest(n: i32, first_player: i32, second_player: i32) -> Vec<i32> {
    ...
    let (earliest, latest) = dp(n as usize, first, second, &mut f, &mut g);
    vec![earliest, latest]
}
...
fn dp(n: usize, first: usize, second: usize, f: &mut [[[i32; 30]; 30]; 30], g: &mut [[[i32; 30]; 30]; 30]) -> (i32, i32) {
    ...
    if first + second > n + 1 {
        let (x, y) = dp(n, n + 1 - second, n + 1 - first, f, g);
        f[n][first][second] = x;
        g[n][first][second] = y;
        return (x, y);
    }
    ...
    if second <= n_half {
        ...
        for i in 0..first {
            for j in 0..(second - first) {
                let (x, y) = dp(n_half, i + 1, i + j + 2, f, g);
        ...
        for i in 0..first {
            for j in 0..(s_prime - first) {
                let (x, y) = dp(n_half, i + 1, i + j + mid + 2, f, g);
                earliest = min(earliest, x);
                latest = max(latest, y);
            }
        }
